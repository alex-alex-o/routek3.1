...
class Users extends MY_Controller {
	public function __construct(){
		...
		public function datatable_json(){				   					   
			...
			{  
				$status = ($row['is_active'] == 0)? 'неактивен': 'активен'.'<span>';
				...
				$data[]= array(
					...
					'<a title="Просмотр" class="view btn btn-sm btn-info" href="'.base_url('admin/users/edit/'.$row['id']).'"> <i class="material-icons">visibility</i></a>
					<a title="Редактировать" class="update btn btn-sm btn-primary" href="'.base_url('admin/users/edit/'.$row['id']).'"> <i class="material-icons">edit</i></a>
					<a title="Удалить" class="delete btn btn-sm btn-danger '.$disabled.'" data-href="'.base_url('admin/users/del/'.$row['id']).'" data-toggle="modal" data-target="#confirm-delete"> <i class="material-icons">delete</i></a>
					',
		...
		public function del($id = 0){
			...
			$this->session->set_flashdata('msg', 'Пользователь удалён!');
			redirect(base_url('admin/users'));
		}
	}
?>