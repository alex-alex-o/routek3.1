...
class Machines extends MY_Controller {
    ...
    public function index() {
        $data['view'] = 'admin/machines/machine_list';
    ...
    public function datatable_json(){				   					   
        ...
        {  
            $data[]= array(
                ...
                $row['quantity'],
                '
                <a title  = "Просмотр"       class = "view btn btn-sm btn-info"      href="'.base_url('admin/machines/edit/'.$row['id']).'"> <i class="material-icons">visibility</i></a>
                <a  title = "Редактировать" class = "update btn btn-sm btn-primary" href="'.base_url('admin/machines/edit/'.$row['id']).'"> <i class="material-icons">edit</i></a>
                <a  title = "Удалить"       class = "delete btn btn-sm btn-danger"  data-href="'.base_url('admin/machines/del/'.$row['id']).'" data-toggle="modal" data-target="#confirm-delete"> <i class="material-icons">delete</i></a>
                ',
            );
        }
    ...
    public function add() {
        ...
        if($this->input->post('submit')) {
            $this->form_validation->set_rules('machine_id', 'Тип оборудования', 'trim|required');
            $this->form_validation->set_rules('name',       'Название',         'trim|min_length[3]|required');
            ...
            if ($this->form_validation->run() == FALSE) {
                    $data['view'] = 'admin/machines/machine_add';
                    $this->load->view('layout', $data);
            } else {
                $data = array(
    ...
    public function edit($id = 0) {
        ...
        if($this->input->post('submit')) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $data['view'] = 'admin/machines/machine_edit';
