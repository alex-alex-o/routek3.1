...
class Offers extends MY_Controller {
    ...
    public function index() {
        $offers = $this->order_model->get_company_offers();
        $data['actualOrders'] = $offers;
        // Словарь файлов для заказа
        $files = array();
        foreach($offers as $offer) {
            $files[$offer["id"]] = $this->order_model->get_order_files($offer["id"]);
        }
        $data['files'] = $files;
        $data['view']  = 'company/offers/offer_cards';    
        
        $this->load->view('layout', $data);
    }

    public function view($id) {
        $data['view']       = 'company/offers/offer_view';
    ...
    public function datatable_json($companyID){				   					   
        ...
        {  
            ...
            $data[]= array(
                ...
                $row["lead_time"],
                '
                <a title  = "Просмотр"       class = "view btn btn-sm btn-info"     href="'.base_url('company/offers/view/'.$row['id']).'"> <i class="material-icons">visibility</i></a>
                ',
            );
        }
    ...
    public function send_to_user($id) {
        ...
        redirect("company/offers/view/$id");
    }
    
    public function refuse($id) {
        $this->offer_model->company_refuse($id);
        redirect("company/offers/index");
    }
