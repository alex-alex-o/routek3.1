    ...
    <div class="card mb-4">    
        <div class="card-body d-flex flex-wrap justify-content-between">
            ...
            <div class="align-self-center mx-2">
                <a href = "/company/orders/view/<?php echo $order["public_id"]; ?>" class="btn btn-sm btn-primary waves-effect">Детали</a>
            </div>
      </div>
    </div>
<?php endforeach; ?>
...
<script>
    document.getElementById('orderActuality').onchange = function() {
        window.location = "<?= base_url() ?>company/orders/index/" + this.value + "/" + document.getElementById('orderType').value;
    };
    
    document.getElementById('orderType').onchange = function() {
        window.location = "<?= base_url() ?>company/orders/index/" + document.getElementById('orderActuality').value + "/" + this.value;
    };
</script>